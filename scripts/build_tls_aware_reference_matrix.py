...
OUT = Path(r'E:/GBM/results/reference_rebuild/tls_aware_reference_matrix')
...
BG_MAP = {
    ...
    'Angiogenic TAMs': 'Macrophage',
    'Anti-inflammatory TAMs': 'Macrophage',
    'Astrocyte-like TAMs': 'Macrophage',
    'Interferon TAMs': 'Macrophage',
    'Pro-inflammatory TAMs': 'Macrophage',
    'Proliferative TAMs': 'Macrophage',
    'RTN1+ TAMs': 'Macrophage',
    'Resident BAM TAMs': 'Macrophage',
    'Resident-TAMs': 'Macrophage',
    'Stress-response TAMs': 'Macrophage',
    'Ambiguous (TAMs)': 'Macrophage',
    'AC progenitor-like': 'Glioma',
    'AC-gliosis-like': 'Glioma',
    'Gliosis-like': 'Glioma',
    'NPC-neuronal-like': 'Glioma',
    'Proliferative AC-OPC-like': 'Glioma',
    'Proliferative NPC-OPC-like': 'Glioma',
    'Proliferative nIPC-like': 'Glioma',
    'Astrocytes': 'Glial',
    'OPC-like': 'Glial',
    'OPC-NPC-like': 'Glial',
    'OPC-neuronal-like': 'Glial',
    'OPCs': 'Glial',
...
FDC_MARKERS = ['CR2', 'FCER2', 'CXCL13', 'C7', 'CLU', 'FDCSP']
...
fdc_score, fdc_detect = score_panel(vasc_df, FDC_MARKERS)
vasc.obs['hev_score'] = hev_score.values
vasc.obs['hev_detect'] = hev_detect.values
vasc.obs['fdc_score'] = fdc_score.values
vasc.obs['fdc_detect'] = fdc_detect.values
endo_mask = vasc.obs['annotation_granular'].astype(str).str.contains('Endothelial', regex=False)
hev_cut = float(np.quantile(vasc.obs.loc[endo_mask, 'hev_score'], 0.90)) if endo_mask.sum() > 0 else np.inf
fdc_cut = float(np.quantile(vasc.obs['fdc_score'], 0.97)) if vasc.n_obs > 0 else np.inf
vasc.obs['ref_label'] = 'perivascular_stromal'
vasc.obs.loc[endo_mask, 'ref_label'] = 'vascular_endothelial'
hev_mask = endo_mask & (vasc.obs['hev_score'] >= hev_cut) & (vasc.obs['hev_detect'] >= 2)
fdc_mask = (vasc.obs['fdc_score'] >= fdc_cut) & (vasc.obs['fdc_detect'] >= 2)
vasc.obs.loc[hev_mask, 'ref_label'] = 'HEV-like_endothelial'
vasc.obs.loc[fdc_mask, 'ref_label'] = 'FDC'
vasc.obs['ref_source'] = 'vascular_block'
for lab in ['HEV-like_endothelial', 'FDC', 'vascular_endothelial', 'perivascular_stromal']:
    sub = vasc[vasc.obs['ref_label'] == lab].copy()
    if sub.n_obs == 0:
        continue
    if lab in {'vascular_endothelial', 'perivascular_stromal'} and sub.n_obs > 350:
        take = RNG.choice(np.arange(sub.n_obs), size=350, replace=False)
        sub = sub[take].copy()
    parts.append(sub)
    qc_rows.append({
        'ref_label': lab,
        'n_cells': int(sub.n_obs),
        'source': 'vascular_block',
        'source_labels': summarize_sources(sub.obs),
        'mean_hev_score': float(sub.obs['hev_score'].mean()) if 'hev_score' in sub.obs else np.nan,
        'mean_fdc_score': float(sub.obs['fdc_score'].mean()) if 'fdc_score' in sub.obs else np.nan,
    })
...
treg_cut = float(np.quantile(cd4.obs['treg_score'], 0.90)) if cd4.n_obs > 0 else np.inf
...
for lab in ['Treg', 'Tfh-like_CD4', 'CD4_T']:
    ...
    qc_rows.append({
        'ref_label': lab,
        'n_cells': int(sub.n_obs),
        'source': 'cd4_treg_block',
        'source_labels': summarize_sources(sub.obs),
        'mean_tfh_score': float(sub.obs['tfh_score'].mean()) if 'tfh_score' in sub.obs else np.nan,
        'mean_treg_score': float(sub.obs['treg_score'].mean()) if 'treg_score' in sub.obs else np.nan,
    })
...
parts.append(cdc)
qc_rows.append({
    'ref_label': 'cDC_or_mature_DC',
    'n_cells': int(cdc.n_obs),
    'source': 'dendritic_block',
    'source_labels': summarize_sources(cdc.obs),
    'mean_dc_score': float(cdc.obs['dc_score'].mean()),
})
...
X = merged.X.toarray() if hasattr(merged.X, 'toarray') else np.asarray(merged.X)
label_order = sorted(merged.obs['ref_label'].unique())
gene_means = []
for lab in label_order:
    idx = np.where(merged.obs['ref_label'].values == lab)[0]
...
merged.write(OUT / 'tls_aware_reference_cells.h5ad', compression='gzip')
gene_means.to_csv(OUT / 'tls_aware_reference_gene_means.csv.gz')
label_counts.to_csv(OUT / 'tls_aware_reference_label_counts.csv', index=False)
qc.to_csv(OUT / 'tls_aware_reference_qc_summary.csv', index=False)

print('saved_h5ad', OUT / 'tls_aware_reference_cells.h5ad')
print('shape', merged.shape)
print('labels', label_counts.to_dict(orient='records'))
print('genes_matrix', gene_means.shape)
