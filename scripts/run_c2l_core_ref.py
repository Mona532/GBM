"""Run cell2location with 17-type core reference on all 142 GBM samples."""
import pandas as pd, numpy as np, anndata as ad, cell2location, torch, scanpy as sc
from pathlib import Path
import warnings; warnings.filterwarnings("ignore")
torch.set_float32_matmul_precision("medium")

REF = Path(r"E:/GBM/results/reference_rebuild/core_ref_v1")
sig = pd.read_csv(REF / "signatures.csv", index_col=0)
genes = [x.strip() for x in (REF / "selected_genes.txt").read_text().splitlines() if x.strip()]
accel = "gpu" if torch.cuda.is_available() else "cpu"
SKIP_DMG = {"GSE194329_DMG1","GSE194329_DMG2","GSE194329_DMG3","GSE194329_DMG4","GSE194329_DMG5"}
OUT = Path(r"E:/GBM/results/c2l_core_v1"); OUT.mkdir(parents=True, exist_ok=True)
