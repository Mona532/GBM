...
OUT = Path(r'E:/GBM/results/reference_rebuild/tls_aware_reference_matrix_v2')
...
for lab in ['HEV-like_endothelial', 'vascular_endothelial', 'perivascular_stromal']:
    ...
    parts.append(sub)
    qc_rows.append({'ref_label': lab, 'n_cells': int(sub.n_obs), 'source': 'vascular_block', 'source_labels': summarize_sources(sub.obs), 'mean_hev_score': float(sub.obs['hev_score'].mean())})
...
merged.write(OUT / 'tls_aware_reference_cells_v2.h5ad', compression='gzip')
gene_means.to_csv(OUT / 'tls_aware_reference_gene_means_v2.csv.gz')
label_counts.to_csv(OUT / 'tls_aware_reference_label_counts_v2.csv', index=False)
qc.to_csv(OUT / 'tls_aware_reference_qc_summary_v2.csv', index=False)

print('saved_h5ad', OUT / 'tls_aware_reference_cells_v2.h5ad')
