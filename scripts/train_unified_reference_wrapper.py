...
train_kwargs = dict(max_epochs=args.max_epochs, batch_size=2048, train_size=1, accelerator='gpu' if torch.cuda.is_available() else 'cpu')
for k, v in [('enable_progress_bar', False), ('progress_bar_refresh_rate', 0)]:
    try:
        model.train(**train_kwargs, **{k: v})
        used_train = {**train_kwargs, k: v}
        break
    except TypeError:
        continue
else:
    model.train(**train_kwargs)
    used_train = train_kwargs
print('used_train_kwargs', used_train)
model.save(outdir / 'reference_model', overwrite=True)

posterior_base = dict(num_samples=1000, batch_size=2048)
try:
    adata = model.export_posterior(adata, sample_kwargs={**posterior_base, 'progress_bar': False})
    used_post = {**posterior_base, 'progress_bar': False}
except TypeError:
    adata = model.export_posterior(adata, sample_kwargs=posterior_base)
    used_post = posterior_base
print('used_posterior_kwargs', used_post)
